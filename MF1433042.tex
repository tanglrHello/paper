...
\subsection{篇章关系分类体系}
篇章关系的类别实际上就是连接篇章的连接词的类型（包括添加的隐式连接词），主要可以分为四大类：因果类、转折类、并列类、解说类。每一类又可以细分为不同的类型，总共有17个小类，每个类型都包含若干连接词，有的连接词可以术语不同关系类型。关系分类体系结构如图\ref{relationtype}所示。
...
\begin{enumerate}
  ...
  \item IP\_COORD：所在选项可拆分，逗号之前的IP的左边界为公共部分右边界，如“图中@\textcircled{1}地/风向偏北，风力逐渐减弱”。
